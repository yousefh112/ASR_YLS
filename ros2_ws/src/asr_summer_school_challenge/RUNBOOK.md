...
| Symptom | Cause | Fix |
|---|---|---|
| `/map has published nothing`; bringup log shows `LaserRangeScan contains N range readings, expected M` | The real LDS-02 emits a varying number of readings per scan (measured 203-209). Karto fixes the count from its first scan and rejects every scan that differs. Gazebo cannot show this | `scan_preprocess` resamples every scan onto a fixed grid. The bringup log must show `resampling every scan to a fixed N`. If it does not, the node running is not ours or is out of date: `deploy_to_robot.sh` |
| Camera up (`RealSense Node Is Up!`) but `/camera/detections` silent, zero tags | AprilTag subscribed to a topic nothing publishes. On nuc11 the RealSense node is `/camera`, publishing `/camera/color/image_raw`. `ros2 topic list` shows a topic even when it only has a subscriber, so both names appear and look fine | Check by publisher count, not by name: `ros2 topic info /camera/color/image_raw` must show `Publisher count: 1` and AprilTag as a subscriber. `apriltag.launch.py` remaps to that absolute name |
| Detections silent, topics match | `apriltag_ros` defaults `qos_profile` to RELIABLE; the RealSense publishes BEST_EFFORT, so nothing is delivered and nothing errors | `config/apriltag.yaml` sets `qos_profile: sensor_data`. Read-only parameter: restart the bringup after changing it |
